...
do
    ...
    ncks -O -d time,-1,-1,1 LIS_output/LIS.CABLE.2001120100.d01.nc LIS.HIST.nc
    ncwa -O -C -a time LIS.HIST.nc tmp.nc

    echo ncks -A -v TBOT,GREENNESS /short/w35/ccc561/WRF/run_nu-wrf_v8/test_1year/LIS_offline/lis_input.d${dom}.nc tmp.nc
    ncks -A -v TBOT,GREENNESS -d month,2,2 /short/w35/ccc561/WRF/run_nu-wrf_v8/test_1year/LIS_offline/lis_input.d${dom}.nc tmp.nc 
    if [ "$?" -ne "0" ]; then
    ...
    # List throug variables and add scale_factor and add_offset attributes
